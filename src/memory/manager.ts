...
export class MemoryIndexManager extends MemoryManagerEmbeddingOps implements MemorySearchManager {
  ...
  async search(
    ...
    opts?: {
      ...
      minScore?: number;
      sessionKey?: string;
      /** Path-based pre-filter applied before MMR/scoring. */
    ...
    const applyPathFilter = <T extends { path: string }>(results: T[]): T[] =>
      opts?.pathFilter ? results.filter((r) => opts.pathFilter!(r.path)) : results;
    ...
    if (!this.provider) {
      ...
      const merged = applyPathFilter(
        [...seenIds.values()].toSorted((a, b) => b.score - a.score),
      );
    ...
    // Apply path filter before MMR/hybrid merge to ensure session-scoped
    // results are not eliminated by cross-session duplicates.
    const filteredVector = applyPathFilter(vectorResults);
    const filteredKeyword = applyPathFilter(keywordResults);

    if (!hybrid.enabled || !this.fts.enabled || !this.fts.available) {
      return filteredVector.filter((entry) => entry.score >= minScore).slice(0, maxResults);
    }
    ...
    if (strict.length > 0 || filteredKeyword.length === 0) {
